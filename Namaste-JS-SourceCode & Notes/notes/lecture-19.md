...
const sumOfElem = arr.reduce(function (accumulator, current) {
  ...
  // In this case accumulater gets updated in each iteration and  returns a arr elements sum value at Last.
...
// acc is just a label which represents the accumulated value till now,
...
const report = users.reduce((acc, curr) => {
	...
	return acc;  // Updating the acc object everytime in every iteration and finally a scalar value / single modified or desired object is returned. 
}, {})
